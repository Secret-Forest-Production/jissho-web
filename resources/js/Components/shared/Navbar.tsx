...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white backdrop-blur-md border-b border-gray-100">
        ...
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden lg:flex items-center gap-8">
            <div className="flex items-center gap-8 mr-4">
              {navLinks.map((link) => {
                const isItemActive = url === link.href || (link.href !== '/' && url.startsWith(link.href));
                ...
                return (
                  ...
                  >
                    ...
                    {/* Underline Animasi */}
                    <span className={`absolute bottom-0 left-0 h-0.5 bg-red-normal transition-all duration-300
      ...
      {/* Mobile Menu Overlay */}
      <div className={`lg:hidden absolute top-20 left-0 w-full bg-white border-b border-gray-100 transition-all duration-500 overflow-hidden ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
            ...
            return (
              <Link
                ...
                className={`px-4 py-3 rounded-lg font-medium transition-all ${
                  isItemActive ? 'bg-red-light/40 text-red-normal' : 'text-blue-dark/80 hover:bg-red-light/20'
                }`}
              >
                {link.label}
              </Link>
